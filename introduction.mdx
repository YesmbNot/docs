---
title: "Introduction to ApiTwitter — Twitter/X REST API"
description: "ApiTwitter is a third-party Twitter/X REST API with 56 endpoints. Pay-per-use pricing from $0.14/1K calls, unlimited write throughput and up to 100 QPS on reads, no X developer approval needed. Access tweets, users, search, DMs, lists, communities, topics. Python & JS SDKs."
seo: "ApiTwitter introduction Twitter API alternative X API documentation"
icon: "book"
---
...
<CardGroup cols={4}>
  ...
  <Card title="No Write Limits" icon="bolt">
    **Unlimited QPS on writes.** Reads scale to **100 req/sec**.
  </Card>
...
<Steps>
  <Step title="Create an Account">
    Register at [apitwitter.com](https://www.apitwitter.com/register) to get your API key.
  </Step>
...
Because writes run on your credentials, they carry **no QPS limit** from us. Reads share our pool, so they are rate-limited by tier — see [Rate Limits](/rate-limits).
</Info>
...
<AccordionGroup>
  <Accordion title="What is ApiTwitter?">
  ...
  <Accordion title="Do I need an X/Twitter developer account?">
  ...
  <Accordion title="How much does ApiTwitter cost?">
    ApiTwitter is pay-per-use with no monthly subscription. Prices start at $0.05 per 1,000 calls for session verification, $0.14/1K for tweet reads, $0.16/1K for user profiles, and $2.25/1K for write operations. Credits never expire. Minimum top-up is $10.
  </Accordion>
  ...
  <Accordion title="How fast is the API?">
    Average response time is under 700ms. **Write endpoints (POST/DELETE) have no QPS limit at all** — they run on your own accounts, so throughput scales with how many accounts you use. Read endpoints (GET) are tiered by balance: 0.2 QPS on the free tier, up to 100 QPS at $200+. See [Rate Limits](/rate-limits).
  </Accordion>
