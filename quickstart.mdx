---
title: "Quickstart Guide — ApiTwitter Twitter/X API"
description: "Get started with ApiTwitter in under 5 minutes. Get your API key, make your first Twitter/X REST API call with curl, Python, or JavaScript. Base URL: api.apitwitter.com. Code examples included."
seo: "ApiTwitter quickstart tutorial Twitter API getting started"
icon: "rocket"
---

## Prerequisites

Before making your first request, you need:

1. An ApiTwitter account — [register here](https://www.apitwitter.com/register)
2. Credits on your balance — minimum $10 top-up

For **GET** read endpoints, that's all you need. For **POST** endpoints (which give you full control with your own cookies), you also need:

3. Your Twitter cookies (`ct0` and `auth_token`)
4. A proxy URL

## Step 1: Get Your API Key

After registration, your API key is automatically created. Find it in your [Dashboard → API Key](https://www.apitwitter.com/api-keys) page.

Your key looks like: `tda_k8x2mN3s_...`

## Step 2: Get Your Twitter Cookies

1. Open [x.com](https://x.com) and log in
2. Open DevTools (F12) → **Application** → **Cookies** → `https://x.com`
3. Copy the values of `ct0` and `auth_token`
4. Format them as: `ct0=YOUR_CT0;auth_token=YOUR_AUTH_TOKEN`

## Step 3: Make Your First Request

### Option A: GET (simplest — uses server pool)

<CodeGroup>

```bash cURL
curl "https://api.apitwitter.com/twitter/user/elonmusk" \
  -H "X-API-Key: tda_your_key_here"
```

```python Python
import requests

response = requests.get(
    "https://api.apitwitter.com/twitter/user/elonmusk",
    headers={"X-API-Key": "tda_your_key_here"}
)
print(response.json())
```

```javascript JavaScript
const response = await fetch(
  "https://api.apitwitter.com/twitter/user/elonmusk",
  { headers: { "X-API-Key": "tda_your_key_here" } }
);
const data = await response.json();
console.log(data);
```

</CodeGroup>

### Option B: POST (your own cookies & proxy)

<CodeGroup>

```bash cURL
curl -X POST "https://api.apitwitter.com/twitter/user/elonmusk" \
  -H "X-API-Key: tda_your_key_here" \
  -H "Content-Type: application/json" \
  -d '{"cookie": "ct0=abc123;auth_token=xyz789", "proxy": "http://user:pass@proxy:8080"}'
```

```python Python
import requests

response = requests.post(
    "https://api.apitwitter.com/twitter/user/elonmusk",
    headers={"X-API-Key": "tda_your_key_here"},
    json={
        "cookie": "ct0=abc123;auth_token=xyz789",
        "proxy": "http://user:pass@proxy:8080"
    }
)
print(response.json())
```

```javascript JavaScript
const response = await fetch(
  "https://api.apitwitter.com/twitter/user/elonmusk",
  {
    method: "POST",
    headers: {
      "X-API-Key": "tda_your_key_here",
      "Content-Type": "application/json"
    },
    body: JSON.stringify({
      cookie: "ct0=abc123;auth_token=xyz789",
      proxy: "http://user:pass@proxy:8080"
    })
  }
);
const data = await response.json();
console.log(data);
```

</CodeGroup>

## Example Response

```json
{
  "data": {
    "id": "44196397",
    "userName": "elonmusk",
    "name": "Elon Musk",
    "screen_name": "elonmusk",
    "followers": 236242504,
    "following": 1292,
    "is_blue_verified": true,
    "description": "...",
    "profile_image_url": "https://pbs.twimg.com/..."
  }
}
```

<Tip>
This request costs **16 credits** (User Profile lookup). Check the [Billing](/billing) page for full pricing details.
</Tip>

## What's Next?

<CardGroup cols={2}>
  <Card title="SDKs" icon="cube" href="/sdks">
    Python & JavaScript client libraries
  </Card>
  <Card title="Authentication" icon="key" href="/authentication">
    Learn about API key authentication
  </Card>
  <Card title="Proxy & Cookies" icon="shield" href="/proxy-and-cookies">
    Understand how to pass proxy and cookies
  </Card>
  <Card title="API Reference" icon="code" href="/api-reference/introduction">
    Explore all 56 endpoints
  </Card>
  <Card title="Billing" icon="credit-card" href="/billing">
    Understand pricing and credits
  </Card>
</CardGroup>

## Quickstart FAQ

<AccordionGroup>
  <Accordion title="How long does it take to get started?">
    Under 5 minutes. Register, get your API key, add credits ($10 minimum), and make your first call. No approval process or waiting period.
  </Accordion>
  <Accordion title="Do I need cookies and a proxy for every request?">
    No. Read-only GET endpoints (user lookups, followers, tweets, search) use a server-side pool — you only need your API key. POST endpoints for write actions (tweet, like, follow, DM) require your own Twitter cookies and proxy.
  </Accordion>
  <Accordion title="What's the base URL for all API requests?">
    All requests go to `https://api.apitwitter.com`. The API key is passed via the `X-API-Key` HTTP header.
  </Accordion>
</AccordionGroup>
